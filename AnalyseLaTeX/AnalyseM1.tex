...
\NouvellePart{Esaces $\mathcal L^p$ et $L^p$}
...
$$\norm{f}_p=0\Leftrightarrow f=0\ \mu_text{-p.p.},$$
...
\begin{Proof}
  On va définir 
  $$M:=\sum_{n\geqslant 0}\norm{f_n}_p\text{ et }M<\infty\text{ par hypothèse},$$
  $$G_n(x)=\sum_{k=0}^n\abs{f_n(x)}$$
  et
  $$G(x)=\sum_{n\geqslant 0}\abs{f_n(x)}.$$
...
\begin{Proof}
  ...
  Donc $(f_n)$ est de Cauchy dans l'espace $C_b(X\backslash N,\norm{\cdot}_\infty)$ où $\norm{\cdot}_\infty$ est la norme $\mathrm{sup}$ habituelle. Cet espace est un Banach (on le montre de la même façon qu'on montre que $(C(K,\K,\norm{\cdot}_\infty)$ est complet) donc $f_n$ converge uniformément sur $X\backslash N$ vers $f\in L^{\infty}$.

  Sur $N$, on pose $f=0$ donc $f$ est toujours dans $L^\infty$, donc on a bien 
...
\begin{RQ}
  Pour $1\leqslant p>+\infty$, $L^p$ est séparable.
\end{RQ}
