...
\begin{Proof}
  ...
  $$\forall (u,v)\in G,\ \varphi(u)+\tilde\varphi(v)=\varphi(u+v)\leqslant p(u+v)\leqslant p(u-x_0)+p(v+x_0)$$
  autrement dit 
...
\begin{Proof}
  ...
  $f$ est holomorphe, $f$ est nulle en $\frac{1}{a_n}$, pour $n\in\N$ et $\frac{1}{a_n}\longrightarrow 0$, donc en vertue du principe des zéros isolés, $f=0$.

  En particulier, pour tout $k\in \N,\ \varphi(e_k)=0$, donc par linéarité, 
  $$\forall P\in \C[X],\ \varphi(P)=0$$
  or $C[X]$ est dense dans $C([-1,1])$ donc $\varphi$ est nulle. A POURSUIVRE.
\end{Proof}
