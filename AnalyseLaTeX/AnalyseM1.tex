...
\Dnote{La (bonne) topologie sur $\S(\R)$ est celle donnée en cours d'\emph{AIN}, donc si on parle de convergence dans $\S(\R)$, il faudra considérer $\S(\R)$ comme un sous-espace d'un $L^p$ et utiliser la norme $\norm{\cdot}_p$, \emph{c.f.} la remarque qui suit\\[2em]}
