...
Ce document est une transciption en \LaTeX\ plus ou moins fidèle au cours d'analyse d'Emmanuel Fricain en première année de master, en 2024-2025.\\[3em]
...
La notation $f:E\hookrightarrow F$ indique que l'application $f$ est injective de $E$ dans $F$, la notation $f:E\twoheadrightarrow F$ indique que l'application $f$ est surjective de $E$ dans $F$.\\
La notation 
$$f_n\ttend f$$
...
Pour $x:N\longrightarrow E$ une suite, la notation $(x_n)_{n\geqslant 0}$ insiste sur l'aspect \og suite\fg\ tandis que la notation $\ens{x_n}_{n\geqslant 0}$ insiste sur l'aspect \og ensemble de points\fg.\\[2em]

Dans le présent document, $\K=\R$ ou $\mathbf C$, $(K,d)$ est un espace métrique compact.\\[2em]
...
\begin{Proof}
  ...
  $$\forall i\in \llbracket 1,N-1\rrbracket,\ \O_{n_i}\subset\O_{n_{i+1}}$$
  donc 
  $$K=\O_{m_n}.$$
  Autrement dit,
...
\begin{corollaire}
  Soient $X$ un espace de Banach, $Y$ un espace vectoriel normé et $\left\{T_i,\ i\in I\right\}$ une suite quelconque d'application linéaires $X\longrightarrow Y$, si 
...
\Dnote{Tous ces résultats sont valables pour la norme 2, pour la norme infinie il n'y a pas nécessairement de convergence. Une conséquence du théorème de Banach-Steinhaus est le résultat suivant :
$\big\{f\in C([-\pi,\pi])\ |\ \underset{n\geqslant 0}{\mathrm{sup}}|S_nf(0)|=+\infty\big\}$
