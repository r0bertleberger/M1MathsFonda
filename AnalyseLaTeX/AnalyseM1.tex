...
\begin{Proof}
  Soit $\mathrm{Id}:(X,||\cdot||_1)\longrightarrow(X,||\cdot||_2)$, cette application est bijective et linéaire, d'après le théorème d'isomorphisme de Banach, c'est un homéomorphisme, donc 
  $$\left\{\begin{array}{l}
    \mathrm{Id} \text{ est continue donc }\exists c>0\tq \forall x\in X, ||x||_2=||\mathrm{Id}x||_2\leqslant c||x||_1\\
    \mathrm{Id}^{-1} \text{ est continue donc }\exists c^{-1}>0\tq \forall x\in X, ||x||_1=||\mathrm{Id}^{-1}x||_1\leqslant c||x||_2
  \end{array}\right.$$
